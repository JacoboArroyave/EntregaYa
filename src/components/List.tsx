                          ...
                          {acciones.map((accion) => (
                            <div className="relative" key={accion.nombre}>
                              ...
                              {activeTooltip?.index === index &&
                                activeTooltip?.action === accion.nombre && (
                  ...
                  >
                    <div className="flex flex-col items-center justify-center py-8">
                      ...
                      <p className="text-amber-600 text-sm mt-1">
                        Intenta agregar nuevos elementos
                      </p>
                    </div>
                  </td>
                </tr>
            ...
            >
              ...
              {acciones.length > 0 && (
                <div className="flex space-x-3 mt-4 justify-end">
                  {acciones.map((accion) => (
                    <button
                      ...
                      className={`text-white rounded-full w-10 h-10 flex items-center justify-center shadow-md transition-all duration-200 transform hover:scale-110 hover:shadow-lg active:scale-95 ${
                        ...
                        accion.nombre.toLowerCase().includes("eliminar")
                          ? "bg-gradient-to-br from-red-400 to-red-600 hover:from-red-500 hover:to-red-700"
                          : accion.nombre.toLowerCase().includes("edit") ||
                            accion.nombre.toLowerCase().includes("editar")
                          ? "bg-gradient-to-br from-amber-400 to-amber-600 hover:from-amber-500 hover:to-amber-700"
                          : accion.nombre.toLowerCase().includes("view") ||
                            accion.nombre.toLowerCase().includes("ver")
                          ? "bg-gradient-to-br from-green-400 to-green-600 hover:from-green-500 hover:to-green-700"
                          : "bg-gradient-to-br from-orange-400 to-orange-600 hover:from-orange-500 to-orange-700"
                      }`}
                    >
                  ...
                  {/* Botón adicional si showFollowOrder es true */}
                  {showFollowOrder && (
                    <button
                      onClick={() => onAccion("seguirPedido", item)}
                      title="Seguir pedido"
                      className="text-white rounded-full w-10 h-10 flex items-center justify-center shadow-md transition-all duration-200 transform hover:scale-110 hover:shadow-lg active:scale-95 bg-gradient-to-br from-blue-400 to-blue-600 hover:from-blue-500 hover:to-blue-700"
                    >
                      <Truck size={18} className="drop-shadow-sm" />
                    </button>
                  )}
                </div>
              )}
