...
const Dashboard = () => {
  const [pieData, setPieData] = useState<{ neighborhoods: string[]; percentages: number[] }>({ neighborhoods: [], percentages: [] });
  const [barData, setBarData] = useState<{ categories: string[]; units: number[] }>({ categories: [], units: [] });
  const [lineData, setLineData] = useState<{ month: string[]; orders: number[] }>({ month: [], orders: [] });
